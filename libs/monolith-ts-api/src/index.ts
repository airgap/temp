import { FromJsonSchema, HandlerModel, JsonSchema, ObjectJsonSchema } from 'from-schema';
...
export const api = Object.fromEntries(
	(Object.keys(monolith) as ContractName[]).map(routeName => {
		...
		return [
			routeName,
			(
				params: ThisRoute extends { request: JsonSchema }
					? ThisRoute['request']
					: never,
			) => {
				const route = monolith[routeName] as HandlerModel;
						...
						.then(res => {
							...
							if (!res.ok) {
								switch (res.status) {
									case 498:
										console.log(
											'Deleting expired token due to 498',
										);
								...
								throw new Error(
									res.statusText ?? 'No status text in error',
								);
							}
							if (!('response' in route && route.response))
								return;
...
export let sessionId: FromJsonSchema<typeof sId> | undefined;
