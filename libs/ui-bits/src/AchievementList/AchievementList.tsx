...
export const AchievementList = ({ game }: { game?: string }) => {
	const [grants, setGrants] = useState<AchievementGrant[]>([]);
	const [dropped, setDropped] = useState(false);
	useEffect(() => {
		if (sessionId)
			api.listenForAchievementGrants({ game }).listen(e =>
				setGrants(g => g.concat(e)),
			);
	}, [game]);
							...
							.map(ach => (
								<AchievementTile
									...
									granted={grants.some(g =>
										g.id.startsWith(ach.id),
									)}
								/>
								))
					) : (
						<h3>
							This game has no achievements yet -- check back
							soon!
						</h3>
					)
				}
