...
export const DynamicPost = ({
	...
	const [author] = useCacheSingleton('users', post.author);
	...
	// const [attachments] = useCacheData('attachments', post.attachments);
			...
			{'attachments' in post && (
				...
				>
					{post.attachments?.map((at) => {
						switch (getSupertypeFromAttachmentId(at)) {
			...
			{!inset && (
				<>
					<span className={styles.PostStats}>
						...
						<ReplyButton onClick={() => setShowReplyer(!showReplyer)} />
