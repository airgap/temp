...
export const FriendInviter = ({
	...
	useEffect(() => {
		...
		api.listMatchProposals({ game }).then(({ proposals }) =>
			setInvites(proposals),
		);
	}, [queried, game]);
	return (
		<>
			...
			{incoming.map(invite => (
				<>
					Incoming from {friendsById[invite.from]?.username}{' '}
			...
			{outgoing.map(invite => (
				<>
					Sent to {friendsById[invite.from]?.username}{' '}
					<Button>Undo</Button>
				</>
			))}
			{remaining.map(friend => (
				<>
					...
					<Button
						onClick={() =>
							api.proposeMatch({ user: friend.id, game })
						}
					>
						Invite
					</Button>
