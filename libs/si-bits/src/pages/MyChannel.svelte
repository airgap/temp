...
<div class={styles.MyChannel}>
  ...
  {#if channel}
    ...
    <span class={styles.shortlines}>
      ...
      />
      {#if sensitives}
        <Button on:click={hideChannelSensitives}>
          {phrasebook.hideStreamInfo}
        </Button>
      {:else}
        <Button
          on:click={showChannelSensitives}
    ...
    <Texticle
      ...
      value={channel.bio}
    />
    <Button on:click={saveChannel} disabled={!savable}>
      Save
    </Button>
    <br />
    <ImageUpload
      image={channel.activeBg ?? channel.awayBg ?? defaultImages.ActiveChannelBackground}
      channelId={channel.id}
    ...
    <ImageUpload
      image={channel.awayBg ?? channel.activeBg ?? defaultImages.AwayChannelBackground}
      channelId={channel.id}
