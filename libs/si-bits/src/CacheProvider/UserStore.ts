...
export function createUserStore() {
  ...
  const pendingRequestsStore = writable<Map<string, Promise<User[]>>>(new Map());
  ...
  function getStoreValue<T>(store: { subscribe: (callback: (value: T) => void) => any }): T {
    let value: T;
  ...
  function getMany(ids: bigint[]): (User | undefined)[] {
    ...
    if (missingIds.length > 0) {
      if (isSSR) {
        // In SSR mode, fetch immediately and synchronously
        const promise = api.getUsers(missingIds)
          .then(fetchedUsers => {
            usersStore.update(map => {
  ...
  async function fetchBatch() {
    ...
    for (let i = 0; i < idsToFetch.length; i += BATCH_SIZE) {
      ...
      const promise = api.getUsers(batchIds)
        .then(fetchedUsers => {
