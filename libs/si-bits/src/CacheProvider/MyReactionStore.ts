 ...
 */
export function createMyReactionStore() {
	// Store all users in a Map
	const internalStore = writable<Map<bigint, string>>(new Map());
	 ...
	 */
	function get(id: bigint): string | undefined {
	 ...
	 */
	function getMany(ids: bigint[]): Map<bigint, string | undefined> {
		...
		return getStoreValue(internalStore);
	}
	...
	/**
	 * Update or add a like to the store
	 */
	function update(post: bigint, type: string) {
		internalStore.update((map) => {
			const newMap = new Map(map); // Create a new map to ensure reactivity
			newMap.set(post, type);
			return newMap;
		});
	}
	 ...
	 */
	function preload(initials: Map<bigint, string>) {
		internalStore.update((map) => {
			return map;
		});
	}
	 ...
	 */
	function hydrate(initial: Map<bigint, string>) {
		try {
...
export const myReactionStore = createMyReactionStore();
