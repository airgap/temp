<script>
  ...
  const MAX_PARTICLES = 1000; // limit total particles
  ...
  class FluidParticle {
    constructor(x, y, color) {
      ...
      this.color = color || `rgb(${59 + Math.random() * 100}, ${130 + Math.random() * 100}, ${246})`;
    }
    ...
    update(containerHeight) {
      ...
      if (this.x <= this.size || this.x >= containerWidth - this.size) {
        this.vx *= -0.8;
        this.x = Math.max(this.size, Math.min(containerWidth - this.size, this.x));
      }
  ...
  function releaseFluid(amount) {
    ...
    for (let i = 0; i < particlesToAdd; i++) {
      ...
      const coords = hilbert(w,~~progress);
      console.log('prog', progress, 'coords', coords)
      const x = 5 + coords.x/w * (containerWidth-10);
      const y = 5 + coords.y/h * (containerHeight-10);
