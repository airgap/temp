...
import { HotPostAggregationService } from '.';

export class HealthCheckServer {
	private app: express.Application;
	private server: any;
	private logger: any;
	private workerService: HotPostAggregationService;

	constructor(port: number = 8080, workerService: HotPostAggregationService) {
		this.app = express();
		this.workerService = workerService;
		this.logger = createLogger({
			level: process.env.NODE_ENV === 'production' ? 'info' : 'debug',
			name: 'reaction-worker-http',
		});
...
export function startHealthCheckServer(
	port: number,
	workerService: HotPostAggregationService,
): HealthCheckServer {
